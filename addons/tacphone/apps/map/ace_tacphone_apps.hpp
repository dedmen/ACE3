class EGVAR(tacphone,apps) {
    class ACE_TacPhone_Map {
        ...
        EGVAR(tacphone,devFile) = QPATHTOF(src\main.tsx); // Path, relative to websource folder, which contains the main entry point (what will be imported into the main UI)
